...
from modules_CMT.extras import next_power_of_2
    ...
    def plot(self, st, outfile='', distance=5e3, ylabel='', xmin=100, xmax=300, legend=False, yaxes=True, title="{net:s}:{sta:s}{ch:s}, {date:s} {time:s}"):
        ...
        if type(st) == stream.Stream:
            for tr in st:
                ...
                if ch == 'E':
                    m = plt.plot(t2, np.real(self.mouse[samp_0:samp_1])*self.amplitude*sin(self.phi)*cos(self.theta)+shifts[ch], colors[ch], linestyle='--')
                elif ch == 'N':
                    m = plt.plot(t2, np.real(self.mouse[samp_0:samp_1])*self.amplitude*cos(self.phi)*cos(self.theta)+shifts[ch], colors[ch], linestyle='--')
                elif ch == 'Z':
                    m = plt.plot(t2, np.real(self.mouse[samp_0:samp_1])*self.amplitude*sin(self.theta)+shifts[ch], colors[ch], linestyle='--', label='simulated')
            ...
            m = plt.plot(t2, np.real(self.mouse[samp_0:samp_1])*self.amplitude, colors[ch], linestyle='--', label='simulated')
