...
from modules_CMT.MT_comps import find_exponent, a2mt, decompose
from modules_CMT.inverse_problem import precalc_greens, calc_taper_window
from modules_CMT.extras import histogram, q_filter, taper_wdw, format_timedelta, align_yaxis, traces_to_stream, rtp2ned
...
def plot_snr(st, outfile='', distance=5e3, ylabel='', arr_t=None, xmin=100, xmax=300, legend=False,
    ...
    colors = {'N': '#8EBA42', 'E': '#988ED5', 'Z': '#E24A33'}
...
def plot_MT(self, outfile='output/centroid.png', facecolor='red'):
    ...
    if outfile:
        plt.savefig(outfile, bbox_inches='tight', pad_inches=0, dpi=100)
    else:
        plt.show()
    plt.clf()
    plt.close()
...
def plot_uncertainty(self, outfile='output/uncertainty.png', n=200, reference=None, best=True, fontsize=None,
    ...
    stdev = {'dc': np.std(dc_perc) / 100, 'clvd': np.std(clvd_perc) / 100, 'iso': np.std(iso_perc) / 100,
             'Mw': np.std(Mw), 't': np.std(shift), 'y': np.std(EW), 'x': np.std(NS), 'z': np.std(depth)}
    ...
    if best:
        mt2 = a2mt(c['a'], system='USE')
        full = beach(mt2, linewidth=lw * 3, facecolor='r', edgecolor='black')
        ax.add_collection(full)
    for a in A:
        mt2 = a2mt(a, system='USE')
        try:
    ...
    s2 = outfile[k:]
    if outfile:
        plt.savefig(s1 + 'MT_Q' + s2, bbox_inches='tight', pad_inches=0, dpi=300)
    else:
        plt.show()
    plt.clf()
    plt.close()
    ...
    plt.ylim(-100 - lw / 2, 100 + lw / 2)
    if best and self.decompose:
        dc = beach((c['s1'], c['d1'], c['r1']), facecolor='r', linewidth=lw * 3, edgecolor='black')
        ax.add_collection(dc)
    for i in range(0, len(strike), 2):
        try:
            dc = beach((strike[i], dip[i], rake[i]), linewidth=lw, nofill=True, edgecolor='black', alpha=0.1)
    ...
    if reference:
        ...
        ax.add_collection(dc)
    if outfile:
        plt.savefig(s1 + 'MT_DCQ' + s2, bbox_inches='tight', pad_inches=0, dpi=300)
    else:
        plt.show()
    plt.clf()
    plt.close()
...
def plot_seismo(self, outfile='output/seismo.png', comp_order='ZNE', cholesky=False, obs_style='black', obs_width=2.5,
                synt_style='r', synt_width=1.65, add_file=None, add_file_style='k:', add_file_width=1.5, add_file2=None,
    ...
    elemse = precalc_greens(self.event, self.centroid, self.targets, self.stations, self.comps_order,
                            self.GF_store_dir)
    try:
        df_GCMT = pd.read_csv(join('/home', 'eq', 'PycharmProjects', 'Q_RMT', 'Analysis', 'thesis', 'NEW_OUTPUT_FINAL',
                              'katalog-final_solution-extendedGCMT.txt'), header=0, delimiter='\t',
                              skipinitialspace=True)
        df_GCMT = df_GCMT[df_GCMT['id'] == int(self.event['id'])]
    except FileNotFoundError:
        pass
    try:
        df_BMKG = pd.read_csv(join('/home', 'eq', 'PycharmProjects', 'Q_RMT', 'Analysis', 'thesis', 'NEW_OUTPUT_FINAL',
                              'katalog-final_solution-extendedSCMTV.txt'), header=0, delimiter='\t',
                              skipinitialspace=True)
        df_BMKG = df_BMKG[df_BMKG['id'] == int(self.event['id'])]
    except FileNotFoundError:
        pass
    # self.event2 = {'id': df_GCMT['GCMT_ID'].values[0], 'lat': df_GCMT['LAT'].values[0], 'lon': df_GCMT['LON'].values[0],
    #                'depth': df_GCMT['DEPTH'].values[0]*1e3, 'mag': df_GCMT['MW'].values[0],
    #                't': UTCDateTime(df_GCMT['ORIGIN TIME'].values[0]), 'remark': '', 'agency': 'GCMT'}

    # elemse2 = precalc_greens(self.event2, self.centroid, self.targets, self.stations, self.comps_order,
    #                         self.GF_store_dir)
    if not df_GCMT.empty:
        # OT = float(UTCDateTime(df_GCMT['ORIGIN TIME'].values[0]))
        LAT = float(df_GCMT['LAT'].values[0])
        LON = float(df_GCMT['LON'].values[0])
        DEPTH = float(df_GCMT['DEPTH'].values[0]) * 1000
        MRR = float(df_GCMT['MRR'].values[0])
        MTT = float(df_GCMT['MTT'].values[0])
        MPP = float(df_GCMT['MPP'].values[0])
        MRT = float(df_GCMT['MRT'].values[0])
        MRP = float(df_GCMT['MRP'].values[0])
        MTP = float(df_GCMT['MTP'].values[0])
        EXP = int(df_GCMT['EXP'].values[0]) - 7
        M = np.array([MRR, MTT, MPP, MRT, MRP, MTP]) * 10 ** EXP
        MT = rtp2ned(M)

        engine = LocalEngine(store_superdirs=[self.GF_store_dir])

        mt_gcmt = MTSource(lat=LAT, lon=LON, depth=DEPTH,
                           mnn=MT[0], mee=MT[1], mdd=MT[2], mne=MT[3], mnd=MT[4], med=MT[5])

        mt_gcmt_traces = []
        response = engine.process(mt_gcmt, self.targets)
        mt_gcmt_traces.append(response.pyrocko_traces())
        elemse2 = traces_to_stream(mt_gcmt_traces, self.comps_order)

    if not df_BMKG.empty:
        # OT = float(UTCDateTime(df_BMKG['ORIGIN TIME'].values[0]))
        LAT = float(df_BMKG['LAT'].values[0])
        LON = float(df_BMKG['LON'].values[0])
        DEPTH = float(df_BMKG['DEPTH'].values[0]) * 1000
        MRR = float(df_BMKG['MRR'].values[0])
        MTT = float(df_BMKG['MTT'].values[0])
        MPP = float(df_BMKG['MPP'].values[0])
        MRT = float(df_BMKG['MRT'].values[0])
        MRP = float(df_BMKG['MRP'].values[0])
        MTP = float(df_BMKG['MTP'].values[0])
        EXP = int(df_BMKG['EXP'].values[0])
        M = np.array([MRR, MTT, MPP, MRT, MRP, MTP]) * 10 ** EXP
        MT = rtp2ned(M)

        engine = LocalEngine(store_superdirs=[self.GF_store_dir])

        mt_bmkg = MTSource(lat=LAT, lon=LON, depth=DEPTH,
                           mnn=MT[0], mee=MT[1], mdd=MT[2], mne=MT[3], mnd=MT[4], med=MT[5])

        mt_bmkg_traces = []
        response = engine.process(mt_bmkg, self.targets)
        mt_bmkg_traces.append(response.pyrocko_traces())
        elemse3 = traces_to_stream(mt_bmkg_traces, self.comps_order)
    ...
    for r in range(self.nr):
        ...
        if not df_GCMT.empty:
            q_filter(elemse2[r], self.stations[r]['fmin'], self.stations[r]['fmax'])
            elemse2[r].trim(UTCDateTime(0) + self.elemse_start_origin)
        if not df_BMKG.empty:
            q_filter(elemse3[r], self.stations[r]['fmin'], self.stations[r]['fmax'])
            elemse3[r].trim(UTCDateTime(0) + self.elemse_start_origin)
    if taper:
    ...
    for sta in plot_stations:
        ...
        for comp in range(3):
            ...
            if not df_GCMT.empty:
                SYNT2[comp] = elemse2[sta][comp].data[0:npts]
            if not df_BMKG.empty:
                SYNT3[comp] = elemse3[sta][comp].data[0:npts]
            for e in range(6):
                SYNT[comp] += elemse[sta][e][comp].data[0:npts] * self.centroid['a'][e, 0]
                # SYNT2[comp] += elemse2[sta][e][comp].data[0:npts] * self.centroid['a'][e, 0]
        comps_used = 0
        for comp in comps:
            weight = self.stations[sta][{0: 'weightZ', 1: 'weightN', 2: 'weightE'}[comp]]
            synt = SYNT[comp] * weight
            if not df_GCMT.empty:
                synt2 = SYNT2[comp] * weight
            if not df_BMKG.empty:
                synt3 = SYNT3[comp] * weight
            # if no_filter:
            # D = np.empty(NPTS)
            # for i in range(NPTS):
            # if i+SHIFT >= 0:
            ...
            # else:
            d = data[sta][comp][0:len(t)] * weight
            if cholesky and self.stations[sta][{0: 'useZ', 1: 'useN', 2: 'useE'}[comp]]:
                if self.LT3:
                    ...
                    for COMP in range(3):
                        ...
                        if not df_GCMT.empty:
                            synt2 += np.dot(self.LT3[sta][y1:y2, x1:x2], SYNT2[COMP])
                        if not df_BMKG.empty:
                            synt3 += np.dot(self.LT3[sta][y1:y2, x1:x2], SYNT3[COMP])
                else:
                    d = np.dot(self.LT[sta][comp], d)
                    synt = np.dot(self.LT[sta][comp], synt)
                    if not df_GCMT.empty:
                        synt2 = np.dot(self.LT[sta][comp], synt2)
                    if not df_BMKG.empty:
                        synt3 = np.dot(self.LT[sta][comp], synt3)
                comps_used += 1
            c = comps.index(comp)
            # if no_filter:
            # ax[r,c].plot(T,D, color='k', linewidth=obs_width)
            if not df_GCMT.empty and not df_BMKG.empty:
                max_amp = max(max(abs(d)), max(abs(synt)), max(abs(synt2)), max(abs(synt3))) * 1.4
            elif not df_GCMT.empty:
                max_amp = max(max(abs(d)), max(abs(synt)), max(abs(synt2))) * 1.4
            elif not df_BMKG.empty:
                max_amp = max(max(abs(d)), max(abs(synt)), max(abs(synt3))) * 1.4
            else:
            ...
            if taper:
                ...
                if not df_GCMT.empty:
                    synt2 = synt2 * taper_window[:, 3 * sta + comp]
                if not df_BMKG.empty:
                    synt3 = synt3 * taper_window[:, 3 * sta + comp]
                ...
                if self.stations[sta][{0: 'useZ', 1: 'useN', 2: 'useE'}[comp]] or not cholesky:
                    ...
                    if self.stations[sta][{0: 'useZ', 1: 'useN', 2: 'useE'}[comp]]:
                        ...
                        l_us, = ax[r, c].plot(t, d, 'gray', linestyle=':', linewidth=obs_width)
                ...
                if self.stations[sta][{0: 'useZ', 1: 'useN', 2: 'useE'}[comp]]:
                    ...
                    if not df_GCMT.empty:
                        l_s2, = ax[r, c].plot(t, synt2, 'orange', linewidth=1)
                    if not df_BMKG.empty:
                        l_s3, = ax[r, c].plot(t, synt3, 'blue', linewidth=1)
                    #    d_max = max(max(synt), -min(synt), d_max)
                else:
                    if not cholesky:
                        ax[r, c].plot(t, synt, color=synt_style, linestyle=':', linewidth=1)
                        if not df_GCMT.empty:
                            ax[r, c].plot(t, synt2, 'orange', linestyle=':', linewidth=1)
                        if not df_BMKG.empty:
                            ax[r, c].plot(t, synt3, 'blue', linestyle=':', linewidth=1)
                        # if not df_GCMT.empty:
                        #     l_us2, = ax[r, c].plot(t, synt2, color='#8C8C8C', linewidth=2)
            ...
            #     else:
            #         if not cholesky:
            #             ax[r, c].plot(t, synt, color='#CDB79E', linewidth=2)
            if add_file:
    ...
    if taper:
        if l_s2 and l_s3:
            if l_us:
                ea.append(
                    f.legend((l_d, l_s, l_s2, l_s3, l_us, l_td),
                             ('obs_data', 'modeled solution', 'modeled GCMT', 'modeled BMKG-MT',
                              'not used data', 'taper window'),
                             loc='lower center',
                             bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=6, numpoints=1,
                             fontsize=12, fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(
                    f.legend((l_d, l_s, l_s2, l_s3, l_td),
                             ('obs_data', 'modeled solution', 'modeled GCMT', 'modeled BMKG-MT', 'taper window'),
                             loc='lower center', bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)),
                             ncol=5, numpoints=1, fontsize=20,
                             fancybox=True, handlelength=3))  # , borderaxespad=0.1
        elif l_s2:
            if l_us:
                ea.append(
                    f.legend((l_d, l_s, l_s2, l_us, l_td),
                             ('obs_data', 'modeled solution', 'modeled GCMT',
                              'not used data', 'taper window'), loc='lower center',
                             bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=5, numpoints=1, fontsize=12,
                             fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(
                    f.legend((l_d, l_s, l_s2, l_td),
                             ('obs_data', 'modeled solution', 'modeled GCMT', 'taper window'), loc='lower center',
                             bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=4, numpoints=1, fontsize=20,
                             fancybox=True, handlelength=3))  # , borderaxespad=0.1
        elif l_s3:
            if l_us:
                ea.append(
                    f.legend((l_d, l_s, l_s3, l_us, l_td),
                             ('obs_data', 'modeled solution', 'modeled BMKG-MT',
                              'not used data', 'taper window'), loc='lower center',
                             bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=5, numpoints=1, fontsize=12,
                             fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(
                    f.legend((l_d, l_s, l_s3, l_td),
                             ('obs_data', 'modeled solution', 'modeled BMKG-MT', 'taper window'), loc='lower center',
                             bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=4, numpoints=1, fontsize=20,
                             fancybox=True, handlelength=3))  # , borderaxespad=0.1
        else:
            if l_us:
                ea.append(f.legend((l_d, l_s, l_us, l_td), ('obs_data', 'modeled solution',
                                                            'not used data', 'taper window'),
                                   loc='lower center', bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=4,
                                   numpoints=1, fontsize=22, fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(f.legend((l_d, l_s, l_td), ('obs_data', 'modeled solution', 'taper window'),
                                   loc='lower center', bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=3,
                                   numpoints=1, fontsize=22, fancybox=True, handlelength=3))  # , borderaxespad=0.1
    else:
        if l_s2 and l_s3:
            if l_us:
                ea.append(f.legend((l_d, l_s, l_s2, l_s3, l_us),
                                   ('obs_data-used', 'modeled solution', 'modeled GCMT', 'modeled BMKG-MT',
                                    'not used data', ), loc='lower center',
                                   bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=5, numpoints=1,
                                   fontsize=14, fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(f.legend((l_d, l_s, l_s2, l_s3),
                                   ('obs_data', 'modeled solution', 'modeled GCMT',  'modeled BMKG-MT'),
                                   loc='lower center', bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=4,
                                   numpoints=1, fontsize=20, fancybox=True, handlelength=3))  # , borderaxespad=0.1
        elif l_s2:
            if l_us:
                ea.append(f.legend((l_d, l_s, l_s2, l_us),
                                   ('obs_data', 'modeled solution-used', 'modeled GCMT',
                                    'not used data'), loc='lower center',
                                   bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=4, numpoints=1,
                                   fontsize=14, fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(f.legend((l_d, l_s, l_s2), ('obs_data', 'modeled solution', 'modeled GCMT'), loc='lower center',
                                   bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=3, numpoints=1,
                                   fontsize=20, fancybox=True, handlelength=3))  # , borderaxespad=0.1
        elif l_s3:
            if l_us:
                ea.append(f.legend((l_d, l_s, l_s3, l_us), ('obs_data', 'modeled solution', 'modeled BMKG-MT',
                                   'not used data', ), loc='lower center',
                                   bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=4, numpoints=1,
                                   fontsize=14, fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(f.legend((l_d, l_s, l_s3), ('obs_data', 'modeled solution', 'modeled BMKG-MT'), loc='lower center',
                                   bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=3, numpoints=1,
                                   fontsize=20, fancybox=True, handlelength=3))  # , borderaxespad=0.1
        else:
            if l_us:
                ea.append(f.legend((l_d, l_s, l_us), ('obs_data', 'modeled solution', 'not used data'), loc='lower center',
                                   bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=3, numpoints=1,
                                   fontsize=20, fancybox=True, handlelength=3))  # , borderaxespad=0.1
            else:
                ea.append(f.legend((l_d, l_s), ('obs_data', 'modeled solution'), loc='lower center',
...
def plot_covariance_function(self, outfile='output/covariance.png', comp_order='ZNE', crosscovariance=False,
    ...
    plot_stations, comps, f, ax, ax_l, ea = self.plot_seismo_backend_1(plot_stations, plot_components, comp_order,
                                                                       ...
                                                                       ylabel=None, waveform=False, sharey='all')
...
def plot_noise(self, outfile='output/noise.png', comp_order='ZNE', obs_style='k', obs_width=2, plot_stations=None,
    ...
    plot_stations, comps, f, ax, ax_l, ea = self.plot_seismo_backend_1(plot_stations, plot_components, comp_order,
                                                                       sharey='all', ylabel=self.unit, waveform=False)
...
def plot_spectra(self, outfile='output/spectra.png', comp_order='ZNE', plot_stations=None, plot_components=None):
    ...
    plot_stations, comps, fig, ax, ax_l, ea = self.plot_seismo_backend_1(plot_stations, plot_components, comp_order,
                                                                   ...
                                                                   ylabel='amplitude spectrum', waveform=False)
...
def plot_seismo_backend_1(self, plot_stations, plot_components, comp_order, crosscomp=False, sharey='row',
                          yticks=True, title_prefix='', xlabel='time [s]', ylabel='', waveform=True):
    """
    ...
    for sta in plot_stations:
        ...
        if not waveform:
            ax[r, 0].set_ylabel(data[sta][0].stats.station + u"\n{0:1.0f} km, {1:1.0f}°".
                                format(self.stations[sta]['dist'] / 1000, self.stations[sta]['az']), fontsize=18)
        # SYNT = {}
        # comps_used = 0
        for comp in comps:
            c = comps.index(comp)
...
def plot_seismo_taper_backend_2(self, outfile, plot_stations, comps, ax, yticks=True, extra_artists=None,
    ...
    if outfile:
        if extra_artists:
            plt.savefig(outfile, bbox_extra_artists=extra_artists, bbox_inches='tight', dpi=100)
            # plt.savefig(outfile, bbox_extra_artists=(legend,))
        else:
            plt.savefig(outfile, bbox_inches='tight', dpi=100)
    else:
