...
from modules_CMT.multiprocessings import seedlink_mp
from modules_CMT._pre_process import dist_mag_weight
...
def read_stations(self, filename, min_dist=0, max_dist=180, max_cluster_num=0, delimiter=' '):
    ...
    for index, row in df.iterrows():
        sta = row['sts']
        l = sta.split(':')
        if l[3] == 'BH' or l[3] == 'HH' or l[3] == 'SS' or l[3] == 'SH':
            weightN = weightE = weightZ = 1.
        else:
            weightN = weightE = weightZ = 0.8
        stn = {'code': l[1], 'lat': row['lat'], 'lon': row['lon'], 'network': l[0], 'location': l[2],
               ...
               'useE': False, 'useZ': False, 'weightN': weightN, 'weightE': weightE, 'weightZ': weightZ,
...
def load_waveforms(self, archive_dir, t_before=None, t_after=None, xml_dir=''):
    """
    ...
    :param t_after: length of the record after the event origin time
    :type t_after: float, optional
    :param xml_dir: path of stationXML files
    :type xml_dir: string
    """
    self.logtext['data'] = s = '\nFetching data from Q archive directory.' \
                               '\n\tarchive dir: {0:s}\n\txml dir:  {1:s}'.format(archive_dir, xml_dir)
    self.log('\n' + s)
    ...
    while i < len(self.stations):
        ...
        # filename = os.path.join(archive_dir, '{:s}.mseed'.format(sta))
        ...
        if len(st) == 0:
            self.stations.pop(i)
            self.log('{0:s}:{1:s}:{2:s}: Waveforms not exist on data. Removing station from further '
                     'processing.'.format(net, sta, cha))
            continue
        st = st.select(channel='{:s}*'.format(cha))
        if len(st) == 0:
            self.stations.pop(i)
            self.log('{0:s}:{1:s}: Channel {2:s} not exist in data. Removing station from further '
                     'processing.'.format(net, sta, cha))
            continue
        if len(st) != 3:
            self.stations.pop(i)
        ...
        if not self.data_are_corrected:
            inv = read_inventory(os.path.join(xml_dir, f"{net}.{sta}.xml"))
            self.stations[i]['inv'] = inv.select(time=self.event['t'])
            for tr in data_raw[-1]:
                filt_resp = self.stations[i]['inv'].select(channel=tr.stats.channel)
                if len(filt_resp) == 0:
                    data_raw.pop()
                    self.stations.pop(i)
                    self.log(
...
def load_q_data(self, archive_dir, t_before=None, t_after=None, xml_file=''):
    """
    Loads waveform from local continuous stream for stations listed in ``self.stations``.
    ...
    while i < len(self.stations):
        ...
        if not self.data_are_corrected:
            self.stations[i]['inv'] = inv.select(network=net, station=sta, time=self.event['t'])
            for tr in data_raw[-1]:
                filt_resp = self.stations[i]['inv'] .select(channel=tr.stats.channel)
                if len(filt_resp) == 0:
                    data_raw.pop()
