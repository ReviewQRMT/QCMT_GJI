...
from modules_CMT.extras import q_filter
...
def set_optimum_params(self, outfile=''):
    ...
    self.azimuth_gap = gap, start, end = azimuth_gap(self.list_all_azimuth)  # calculate az gap of all data
    self.lst_qdr_edge = lst_qdr_edge = divide_sector_edge(start, end, self.sector_number)  # get list of sector boundary
    ...
    for i in range(len(lst_qdr_edge) - 1):
        ...
        if len(sel_sta) == 0:
            ...
            select_sta = sorted(sel_sta, key=lambda k: k['weight']+k['weightZ']+k['weightN']+k['weightE'], reverse=True)
