...
import numpy as np
import math
from modules_CMT.extras import lcmm, next_power_of_2
...
def set_grid(self, depth_min=None, depth_max=None, step_x=None, step_z=None):
    """
...
def set_initial_frequencies(self, fmin=None, fmax=None):
    ...
    """
    if not fmin or fmin == 'auto':
        if float(self.event['mag']) < 2:
            fmin = 0.03
        elif float(self.event['mag']) < 3:
            fmin = 0.02
        elif float(self.event['mag']) < 5:
            fmin = 0.015
        else:
            fmin = 0.01
    if not fmax or fmax == 'auto':
...
def set_working_frequencies(self, sampling_fmax=0.125, list_fmin=None, min_range=0.025):
    ...
    if self.minfreq != 'auto':
        list_fmin = [self.minfreq]
    else:
        if float(self.event['mag']) < 1:
            list_fmin = [0.03]
        elif float(self.event['mag']) < 2:
    ...
    if self.maxfreq != 'auto':
        list_fmax = [self.maxfreq]
    else:
        if float(self.event['mag']) < 1:
            list_fmax = [0.1]
        elif float(self.event['mag']) < 2:
...
def set_parameters(self, sampling_fmax=0.125, log=False, init=False, depth_min=None, depth_max=None):
    ...
    if init:
        self.set_working_frequencies(sampling_fmax=sampling_fmax, min_range=0.03)
        self.set_grid(depth_min, depth_max, 3000, 3000)
    else:
