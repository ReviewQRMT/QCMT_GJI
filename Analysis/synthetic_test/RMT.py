...
from synt_CMT.output import write_rmt_config
from synt_CMT.multiprocessings import find_optimum_params_mp
...
class CMT:
    ...
    from synt_CMT._load_data import vars_by_mag, set_event_info, read_stations, create_station_index, \
    ...
    from synt_CMT._config import set_grid, set_time_grid, set_time_window, set_parameters, set_Greens_parameters, \
        set_initial_frequencies, set_working_frequencies, set_working_sampling, config_dict, max_time  # min_time
    from synt_CMT._pre_process import detect_mouse, detect_mouse_mp, snr_filter, calc_snr, correct_data_xml, \
    ...
    from synt_CMT._covariance_matrix import covariance_matrix, covariance_matrix_prenoise, covariance_matrix_residual
    from synt_CMT._inversion import run_inversion, find_best_grid_point, VR_all_components, print_solution, \
        write_solution, print_fault_planes, unused_sts_filter, solution_quality_classification,\
        decompose_fault_planes  # save_seismo
    from synt_CMT._optimization import set_optimum_params, calc_threshold  # find_optimum_params,
    from synt_CMT._plot import plot_MT, plot_seismo, plot_stations, plot_slices, plot_maps, plot_maps_sum, plot_3D, \
    ...
    def load_data(self, lat, lon, dep, mag, origin, remark, list_station_file, seedlink_client=None,
                  fsdn=False, datadir=None, inv_file=None, noise_amp=None):
        ...
        try:
            self.read_stations(filename=list_station_file, min_dist=mindist, max_dist=maxdist, max_cluster_num=6)
            ...
            self.load_q_data(datadir, xml_file=inv_file, noise_amp=noise_amp)
    ...
    def initial_inversion_settings(self):
        ...
        return self.set_parameters(init=True, depth_min=None, depth_max=None)
    ...
    def run_initial_inversion(self, sensitivity_file='', plot=True):
        ...
        if self.use_cov_residual or self.use_cov_noise:
            try:
                self.covariance_matrix(crosscovariance=False, toeplitz=False, correlation=True, save_non_inverted=True,
                                       save_covariance_function=True, normalize=False, init=True)  # TEST1
                # self.covariance_matrix(crosscovariance=False, save_non_inverted=True, save_covariance_function=True,
                #                        init=True)
            except ValueError:
                return False, ''
        # self.decimate_shift()  ##
        ...
        if plot:
            ...
            # self.plot_seismo(outfile=os.path.join(init_dir, 'seismo_cova.png'), cholesky=True)
    ...
    def optimum_inversion_settings(self, sensitivity_file=''):
        ...
        if not self.set_parameters(log=True, depth_min=None, depth_max=None):
            return False
        # self.max_gap, start_gap, end_gap = self.obs_azimuth()
    ...
    def run_final_inversion(self, input_dir, sensitivity_file='', plot=True, save_config=False):
        ...
        if self.use_cov_residual or self.use_cov_noise:
            try:
                self.covariance_matrix(crosscovariance=False, toeplitz=False, correlation=True, save_non_inverted=True,
                                       save_covariance_function=True, normalize=False, plot_each_cov_function=True)  # TEST1
                # self.covariance_matrix(crosscovariance=False, save_non_inverted=True, save_covariance_function=True,
        ...
        if plot:
            ...
            self.plot_seismo(outfile=os.path.join(self.outdir, 'fit_true_waveform.png'), sharey='all')
            self.plot_seismo(outfile=os.path.join(self.outdir, 'fit_true_waveform_row.png'), sharey='row')
            self.plot_seismo(outfile=os.path.join(self.outdir, 'fit_true_waveform_taper.png'), taper=True, sharey='all')
            if self.use_cov_residual or self.use_cov_noise:
                self.plot_seismo(outfile=os.path.join(self.outdir, 'fit_cova_waveform.png'), cholesky=True, sharey='all')
                self.plot_seismo(outfile=os.path.join(self.outdir, 'fit_cova_waveform_row.png'), cholesky=True, sharey='row')
                self.plot_seismo(outfile=os.path.join(self.outdir, 'fit_cova_waveform_taper.png'), taper=True,
