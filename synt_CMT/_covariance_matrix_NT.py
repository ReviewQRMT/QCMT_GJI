...
from synt_CMT.extras import decimate, moving_average_scale, covariance_function, correlation_function
from synt_CMT.multiprocessings import covariance_matrix_prenoise_mp, covariance_matrix_residual_mp


def covariance_matrix(self, crosscovariance=False, toeplitz=False, correlation=False, save_non_inverted=False,
                      save_covariance_function=False, plot_each_cov_function=True, init=False, normalize=False):
    ...
    if self.use_cov_noise:  # and init:  # test5
        try:
            CfD, CdD = self.covariance_matrix_prenoise(crosscovariance=crosscovariance, toeplitz=toeplitz,  # TEST1
    ...
    if self.use_cov_residual and not init:
        try:
            ...
            toeplitz = False
            CfT, CdT = self.covariance_matrix_residual(crosscovariance=crosscovariance, toeplitz=toeplitz,
                                                       save_non_inverted=save_non_inverted, correlation=correlation,
                                                       save_covariance_function=save_covariance_function)
...
#                        CdD[i] = CdD[i] * (Ct_max * CdD[i].max() / Cd_max) / Cd_max
        ...
        for r, Cd, Ct in zip(range(len(self.stations)), CdD, CdT):
            C = Cd + Ct
            # C = np.linalg.inv(np.linalg.inv(Cd) + np.linalg.inv(Ct))
            sta = self.stations[r]
            idx = []
            if sta['useZ']:
                idx.append(0)
