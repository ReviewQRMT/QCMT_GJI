...
from synt_CMT.extras import q_filter, decimate, moving_average_scale, covariance_function, correlation_function
...
def time_shifting_mp(SHIFT, DATA, DATA_orig, starttime, endtime, decimate, elemse_start_origin, npts_slice, event,
                     stations, nr, components, stations_index, max_samprate, taper_perc):
    """
    decimate shift
    :param SHIFT:
    :param DATA:
    ...
    if DATA_orig:
        for st in DATA_orig:
            ...
            # q_filter(st2, fmin, fmax)
            st2.trim(starttime + shift, endtime + shift + 1)  # add 1 s to be sure no index point outside the range
            data_orig.append(st2)
        c = 0
        d_shift_orig = np.empty((components * npts_slice, 1))
        for r in range(nr):
            for comp in range(3):
                if stations[r][{0: 'useZ', 1: 'useN', 2: 'useE'}[comp]]:  # component 'use in inversion'
                    weight = stations[r][{0: 'weightZ', 1: 'weightN', 2: 'weightE'}[comp]]
                    try:
                        d_shift_orig[c * npts_slice:c * npts_slice + npts_slice] = \
                            data_orig[r][comp].data[:npts_slice].reshape(npts_slice, 1) * weight
                    except:
...
def find_optimum_params_mp(stn_num, list_sts, npts_slice, data_raw_shifts, list_models, seismogram, t_min, npts_elemse,
                           ...
                           sensitivity_file, stations, idx_use, idx_weight, taper_perc):
    """
    ...
    Module :class:`multiprocessing` does not allow running function of the same class in parallel,
        so the function :func:`invert` cannot be method of class :class:`synt_CMT` and this wrapper is needed.
    """
    npts = npts_slice
    ...
    for model in list_models:
        ...
        elemse_raw = precalc_greens(event, centroid, targets, [sts], comps_order,
                                    GF_store_dir)
