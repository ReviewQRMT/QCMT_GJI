...
def a2mt(a, system='NEZ'):
    ...
    if system == 'NEZ':
        ...
        return [mt[2], mt[0], mt[1], mt[4], -mt[5], -mt[3]]  # convert to USE coord system
...
def decompose_mopad(mt):
    ...
    :return: dictionary {'dc_perc': double couple percentage, 'clvd_perc': compensated linear vector dipole percentage,
        'iso_perc': isotropic component percentage, 'faultplanes': list of fault planes parameters,
        'moment': scalar seismic moment, 'Mw': moment magnitude :math:`M_W`,
        's1': strike (fault plane 1), 'd1': dip (fault plane 1), 'r1': slip rake (fault plane 1),
        's2': strike (fault plane 2), 'd2': dip (fault plane 2), 'r2': slip rake (fault plane 2)}
    """
...
def angles(n1, n2):
    ...
    :param n1: normals to the fault planes
    :param n2: normals to the fault planes
    :type n1: list or 1-D array of 3 floats
    :type n2: list or 1-D array of 3 floats
    :return: return a tuple of the strike, dip, and rake (one of two possible solutions; the second can be obtained
    by switching parameters ``n1`` and ``n2``)
