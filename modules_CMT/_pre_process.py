...
from modules_CMT.multiprocessings import time_shifting_mp, mouse_mp
from modules_CMT.geometry import azimuth_gap, divide_sector_edge
from modules_CMT.extras import q_filter, next_power_of_2
from modules_CMT.inverse_problem import calc_taper_window
            ...
            try:
                ...
                if (amp > 50e-8) or (amp > 10e-8 and fit > 0.8) or (amp > 7e-8 and fit > 0.9) or \
                    ...
                    self.stations_index['_'.join([stats.network, sta, stats.location,
                                                  stats.channel[0:2]])]['use' + stats.channel[2]] = False
                    self.stations_index['_'.join([stats.network, sta, stats.location,
                                                  stats.channel[0:2]])]['weight' + stats.channel[2]] = self.stations_index['_'.join([stats.network, sta, stats.location,
                                                  stats.channel[0:2]])]['weight' + stats.channel[2]] - 0.0
                    detected = True
                # else:
                #     out += '  ' + sta + ' ' + stats.channel + ': No MOUSE detected, using component in inversion ' \
                #                                               '(onset time: {o:6.1f} s, amplitude: {a:10.2e} ' \
                #                                               'm.s^-2, fit: {f:7.2f})\n'. \
...
def correct_data_xml(self):
    ...
    for i in range(len(self.data_raw) - 1, -1, -1):  # reverse order to delete sts and data if not pass
        ...
        filt_resp = self.stations[i]['inv'].select(location=loc, network=net, station=sta,
                                                   channel=f"{cha}*", time=self.event['t'])
        if len(filt_resp) == 0:
            del self.data_raw[i]
            del self.stations[i]
            # self.data_raw.remove(self.data_raw[i])
...
def decimate_shift_mp(self):
    ...
    if self.threads > 1:
        ...
        results = [pool.apply_async(time_shifting_mp,
                                    args=(SHIFT, self.data, starttime, endtime, decimate, self.elemse_start_origin,
        ...
        for SHIFT in range(self.SHIFT_min, self.SHIFT_max + 1, self.SHIFT_step):
            result = time_shifting_mp(SHIFT, self.data, starttime, endtime, decimate, self.elemse_start_origin,
                                      self.npts_slice, self.event, self.stations, self.nr, self.components,
                                      self.stations_index, self.max_samprate, self.taper_perc)
...
def station_weight(self):
    ...
    :return:
    """
    check_values = ['AAI', 'AAII']  # set priority for colocated sensor
    # Check if all values in check_values are present in the sts list
    all_present = all(value in [stn['code'] for stn in self.stations] for value in check_values)
    ...
    for sts in self.stations:
        ...
        if 'mean_snr' in sts:
            ...
            sts['w_snr'] = sts['mean_snr'] / self.max_snr
        if 'mean_amp' in sts:
            weight -= (sts['mean_amp'] / self.max_amp) * 0.5
            sts['w_namp'] = (sts['mean_amp'] / self.max_amp) * 0.5
        if 'mean_fit' in sts:
            weight -= sts['mean_fit'] * 0.5
            sts['w_nfit'] = sts['mean_fit'] * 0.5

        if all_present and sts['code'] == 'AAII':
            weight -= 0.2
...
def station_selection(self, sector_num=8, numsta_sect=1, outfile=''):
    ...
    for stn in sts:  # get list of az and dist of used station
        ...
        list_all_azimuth.append(stn['az'])
        list_azdist.append((stn['az'], stn['dist'] / 1000, stn['code']))

    self.list_all_azimuth = list_all_azimuth
    ...
    for stn, dt_raw in zip(self.stations.copy(), self.data_raw.copy()):
        ...
        if 'mean_snr' in stn:
            if 'w_snr' in stn:
                out += 'mean_snr={snr:.1f}(w+{w_snr:.1f}), '.format(snr=stn['mean_snr'], w_snr=stn['w_snr'])
            else:
                out += 'mean_snr={snr:.1f}, '.format(snr=stn['mean_snr'])
        if 'mean_amp' in stn:
            if 'w_namp' in stn:
                out += 'mean_mouse_amp={amp:.2e}(w-{w_namp:.1f}), '.format(amp=stn['mean_amp'], w_namp=stn['w_namp'])
            else:
                out += 'mean_mouse_amp={amp:.2e}, '.format(amp=stn['mean_amp'])
        if 'mean_fit' in stn:
            if 'w_nfit' in stn:
                out += 'mean_mouse_fit={fit:.1f}(w-{w_nfit:.1f}), '.format(fit=stn['mean_fit'], w_nfit=stn['w_nfit'])
            else:
