...
def plot_uncertainty(self, outfile='output/uncertainty.png', n=200, reference=None, best=True, fontsize=None,
    ...
    # todo: add fault geometry uncertainty
...
def plot_seismo(self, outfile='output/seismo.png', comp_order='ZNE', cholesky=False, obs_style='k', obs_width=3,
    ...
    if taper:
        plot_stations, comps, f, ax, ax_l, ea = self.plot_seismo_taper_backend_1(
            plot_stations, plot_components, comp_order, sharey=False, xlabel='', ylabel=(self.unit, None)[cholesky],
    ...
    #         title_prefix=('', 'standardized ')[cholesky and self.LT3 != []])
        ...
        plot_stations, comps, f, ax, ax_l, ea = self.plot_seismo_backend_1(
            plot_stations, plot_components, comp_order, sharey=False, ylabel=(self.unit, None)[cholesky],
            title_prefix=('', 'standardized')[cholesky])  # and self.LT3 != []])
    ...
    if taper:
        ea.append(
            f.legend((l_d, l_s, l_td), ('observation', 'synthetic', 'taper window'), loc='lower center',
                     bbox_to_anchor=(0.5, 1. - 0.0066 * len(plot_stations)), ncol=3, numpoints=1, fontsize='medium',
    ...
    if taper:
        self.plot_seismo_taper_backend_2(outfile, plot_stations, comps, ax, extra_artists=ea)
    else:
...
def plot_covariance_function(self, outfile='output/covariance.png', comp_order='ZNE', crosscovariance=False,
    ...
    self.plot_seismo_backend_2(outfile, plot_stations, comps, ax, yticks=True, extra_artists=ea)
...
def plot_seismo_backend_1(self, plot_stations, plot_components, comp_order, crosscomp=False, sharey=False,
    ...
    COMPS = (1, 3)[crosscomp]
    f, ax = plt.subplots(len(plot_stations) * COMPS, len(comps), sharex='all', sharey='none',
...
def plot_seismo_taper_backend_1(self, plot_stations, plot_components, comp_order, crosscomp=False, sharey=True,
                                yticks=True, title_prefix='', xlabel='time [s]', ylabel=''):
    """
    The first part of back-end for functions :func:`plot_seismo`, :func:`plot_covariance_function`,
    :func:`plot_noise`, :func:`plot_spectra`. There is no need for calling it directly.
    ...
    COMPS = (1, 3)[crosscomp]
    f, ax = plt.subplots(len(plot_stations) * COMPS, len(comps), sharex='none', sharey='none',
                         figsize=(len(comps) * 6, len(plot_stations) * 2 * COMPS))
    if len(plot_stations) == 1 and len(comps) > 1:  # one row only
        ax = np.reshape(ax, (1, len(comps)))
    elif len(plot_stations) > 1 and len(comps) == 1:  # one column only
